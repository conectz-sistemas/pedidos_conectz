...
export function ProductDeleteButton({
  ...
  productId: string;
  productName: string;
  /** Se informado, redireciona após excluir (ex: ao excluir na página do produto) */
  ...
  async function handleDelete() {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/${merchantSlug}/products/${productId}`, {
        method: "DELETE",
      });
  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-2">
        <span className="text-xs text-white/70">Excluir &quot;{productName}&quot;?</span>
        <button
          type="button"
