...
export function IngredientGroupDeleteButton({
  merchantSlug,
  groupId,
  groupName,
}: {
  ...
  async function handleDelete() {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/${merchantSlug}/ingredient-groups/${groupId}`, {
        method: "DELETE",
      });
  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-2">
        <span className="text-xs text-white/70">Excluir grupo &quot;{groupName}&quot;?</span>
        <button
          type="button"
